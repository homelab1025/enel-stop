...
pub async fn setup_redis() -> &'static Client {
    INIT.get_or_init(|| async {
        ...
        let redis_container = GenericImage::new("redis", REDIS_TAG)
            ...
            .with_container_name("test-container-redis")
            .start()
            .await
            .unwrap();
        ...
        let redis_client = Client::open(conn_string).expect("Could not connect to own container.");
